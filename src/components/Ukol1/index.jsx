...
export const Ukol1 = () => {
  ...
  const bulbLabel = bulbOn ? 'Vypnout' : 'Zapnout';

  const handleSwitch = () => {
    if (bulbOn === false) {
      const confirm = window.confirm('Opravdu chcete zanout žárovku?');
      if (confirm === true) {
        setBulbOn(true);
      } else {
        setBulbOn(false);
  ...
  return (
    <>
      ...
      <SwitchButton on={bulbOn} onSwitch={handleSwitch} label={bulbLabel} />
    </>
  );
};
